...
<script runat="server">
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        conn = New Data.SqlClient.SqlConnection("Data Source=.\SQLEXPRESS;AttachDbFilename=|DataDirectory|\snapwork.mdf;Integrated Security=True;User Instance=True")
        conn.Open()
        command = New Data.SqlClient.SqlCommand("select count(*) from tchrstunotes where  ClassName='" & d1.Text & "' and Section='" & d2.Text & "' and NotesSubject='" & t2.Text & "'")
        command.Connection = conn
        count = command.ExecuteScalar
        If (count > 0) Then
            MsgBox("This Notes Upload Is Alredy Exists")
            t2.Text = ""
            conn.Close()
        Else
            sql = "insert into tchrstunotes values('" & t1.Text & "','" & UCase(d1.Text) & "','" & UCase(d2.Text) & "','" & UCase(t2.Text) & "','" & t2.Text & "','" & t3.Text & "','" & Label1.Text & "')"
            datareader.InsertCommand = New Data.SqlClient.SqlCommand(sql, conn)
            datareader.InsertCommand.ExecuteScalar()
            MsgBox("Notes Upload Has Been Saved")
            conn.Close()
            t1.Text = ""
            t2.Text = ""
            t3.Text = ""
    ...
    Protected Sub Button5_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        count = command.ExecuteScalar
        If (count > 0) Then
            command = New Data.SqlClient.SqlCommand("select ClassName from tchrstunotes where ClassName='" & d1.Text & "' and Section='" & d2.Text & "' and NotesSubject='" & t2.Text & "'")
            command.Connection = conn
            d1.Text = command.ExecuteScalar
            command = New Data.SqlClient.SqlCommand("select Section from tchrstunotes where ClassName='" & d1.Text & "' and Section='" & d2.Text & "' and NotesSubject='" & t2.Text & "'")
            command.Connection = conn
            d2.Text = command.ExecuteScalar
            command = New Data.SqlClient.SqlCommand("select NotesSubject from tchrstunotes where ClassName='" & d1.Text & "' and Section='" & d2.Text & "' and NotesSubject='" & t2.Text & "'")
            command.Connection = conn
            t2.Text = command.ExecuteScalar
            command = New Data.SqlClient.SqlCommand("select CurrentDate from tchrstunotes where ClassName='" & d1.Text & "' and Section='" & d2.Text & "' and NotesSubject='" & t2.Text & "'")
            command.Connection = conn
            t1.Text = command.ExecuteScalar
            command = New Data.SqlClient.SqlCommand("select NotesDescription from tchrstunotes where ClassName='" & d1.Text & "' and Section='" & d2.Text & "' and NotesSubject='" & t2.Text & "'")
            command.Connection = conn
            t3.Text = command.ExecuteScalar
        Else
            MsgBox("Notes Upload Is Not Find")
            t1.Text = ""
            t2.Text = ""
            t3.Text = ""
        End If
    ...
    Protected Sub Button6_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        conn = New Data.SqlClient.SqlConnection("Data Source=.\SQLEXPRESS;AttachDbFilename=|DataDirectory|\snapwork.mdf;Integrated Security=True;User Instance=True")
        conn.Open()
        command = New Data.SqlClient.SqlCommand("delete from tchrstunotes where ClassName='" & d1.Text & "' and Section='" & d2.Text & "' and NotesSubject='" & t2.Text & "'")
        command.Connection = conn
        command.ExecuteScalar()
        MsgBox("Notes Upload Has Been Deleted")
        t1.Text = ""
        t2.Text = ""
        t3.Text = ""
    End Sub
    ...
    Protected Sub Button7_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        conn = New Data.SqlClient.SqlConnection("Data Source=.\SQLEXPRESS;AttachDbFilename=|DataDirectory|\snapwork.mdf;Integrated Security=True;User Instance=True")
        conn.Open()
        command = New Data.SqlClient.SqlCommand("update tchrstunotes set notesdescription='" & t3.Text & "',pdf='" & pdf & "' where ClassName='" & d1.Text & "'and Section='" & d2.Text & "' and NotesSubject='" & t2.Text & "'")
        command.Connection = conn
        command.ExecuteScalar()
        MsgBox("Notes Upload Has Been Update")
        t1.Text = ""
        t2.Text = ""
        t3.Text = ""
    End Sub
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      ...
      <tr>
        <td colspan="3" bgcolor="#0D152E"><span class="style11">Teacher Notes Upload</span></td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td colspan="2">
            <asp:DropDownList ID="d2" runat="server">
                <asp:ListItem>Slect Any One</asp:ListItem>
      ...
      <tr>
        <td height="31">PDF/Document/Image</td>
        <td colspan="2">
